<?php
	...
	function Dispatching($id_bord,$step,$util,$ville,$bdd){
		...
		$conf = 'Distribution des liasses du Matching au Controle';
		$query = $bdd -> prepare("SELECT VALEUR FROM configuration WHERE LIBELLE = :conf AND VILLE =:ville");
		$query -> bindParam(':conf', $conf, PDO::PARAM_STR);
		$query -> bindParam(':ville', $ville, PDO::PARAM_INT);
		$query -> execute();
		$data = $query -> fetch();
		
		$dispatching = $data['VALEUR'];
		
		$query -> closeCursor();

		if(($dispatching == 0) && ($step == 2)){

			$query1 = $bdd -> prepare("SELECT ID_CERTIF FROM certif_bord WHERE ID_BORD=:bord");
			$query1 -> bindParam(':bord', $id_bord, PDO::PARAM_INT);
			$query1 -> execute();

			while ($data1 = $query1 -> fetch()){

				$query2 = $bdd -> prepare("UPDATE certificat SET ID_USER_PREC=:util, ID_USER_COUR=0 WHERE ID_CERTIF =:certif");
			...
			Journalisation(0,6,$id_bord,0,$bdd);
		...
		if(($dispatching == 1) || ($step == 1) || ($step == 3) || ($step == 4) || ($step == 5) || ($step == 6)){
			...
			$query5 = $bdd->prepare("SELECT ID_UTIL FROM utilisateur WHERE TYPE_COMPTE=2 AND STAT_COMPTE = 0 AND PREM_CONNEXION = 1 AND ID_VILLE=:vil AND ID_UTIL<>:util ORDER BY COMPTE");
			$query5 -> bindParam(':vil', $ville, PDO::PARAM_INT);
			$query5 -> bindParam(':util', $util, PDO::PARAM_INT);
			$query5 -> execute();
			...
			if($rows5 >= 1){
				...
				$query14 -> bindParam(':util', $util, PDO::PARAM_INT);
				$query14 -> execute();
				...
				if($rows14 >= 1){
					...
					$query13 = $bdd -> prepare("UPDATE bordereau SET STATUT = 1 WHERE ID_BORD=:bord");
					$query13 -> bindParam(':bord', $id_bord, PDO::PARAM_INT);
					$query13 -> execute();
					$query13 -> closeCursor();
