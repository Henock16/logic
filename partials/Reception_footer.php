			<footer class="main-footer">
				<div class="pull-right hidden-xs">
					<b>Version</b> 1.0
				</div>
				<strong>Copyright &copy; 2017-2018&nbsp;&nbsp;<a target="_blank" href="http://www.cci.ci">CCI - C&ocirc;te d'Ivoire</a>.</strong> Tous droits r&eacute;serv&eacute;s.
			</footer>		
		</div>
		<!-- ./wrapper -->
		
		<script src="./assets/DataTables/datatables.min.js"></script>
		<script src="./assets/js/jquery.jqGrid.min.js"></script>
		<script src="./assets/js/grid.locale-en.js"></script>
		<script src="./assets/js/bootstrap.min.js"></script>
		<script src="./assets/js/fastclick.js"></script>
		<script src="./assets/js/adminlte.min.js"></script>
		<script src="./assets/js/jquery.sparkline.min.js"></script>
		<script src="./assets/js/jquery-jvectormap-1.2.2.min.js"></script>
		<script src="./assets/js/jquery-jvectormap-world-mill-en.js"></script>
		<script src="./assets/js/jquery.slimscroll.min.js"></script>
		<script src="./assets/js/jquery-ui.js"></script>
		<script src="./assets/js/Widget.js"></script>
		<script src="./assets/js/Timeis.js"></script>
		<script src="./assets/js/Reception.js"></script>
		<script src="./assets/js/Reception-packinglist.js"></script>
		<script src="./assets/js/Reception-archive.js"></script>
		<script src="./assets/js/Reception-init.js"></script>
		<script src="./assets/js/Deconnection.js"></script>

	</body>
</html>
