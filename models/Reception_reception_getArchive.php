<?php
    ...
    if(!empty($_POST['idcert'])){
        ...
        $query1 = $bdd -> prepare("SELECT ID_CERTIF,  NUM_PCKLIST, STATUT, ID_VILLE, A_R, ID_EXP, ID_TYPPROD, DATE_CREATION  FROM certificat WHERE  ((STATUT IN(3,4)) OR (A_R = 1 ))AND ID_CAMP IN (SELECT ID_CAMP FROM campagne WHERE STATUT = 0)AND (ID_VILLE = :ville OR ID_VILLE=:ville2) AND YEAR(DATE_CREATION) = YEAR(NOW()) ORDER BY DATE_CREATION DESC LIMIT 300");
