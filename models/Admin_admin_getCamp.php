<?php
	...
	if(!empty($_POST['idcamp'])){
		
		$tab[0] = 1;
		
		$query = $bdd -> prepare("SELECT DEBUT, FIN FROM campagne WHERE ID_CAMP =:idcamp");
		$query -> bindParam(':idcamp', $_POST['idcamp'], PDO::PARAM_INT);
		$query -> execute();
		...
		$query = $bdd -> query("SELECT ID_CAMP, LIBELLE, DEBUT, FIN, PRODUIT, STATUT, DATE_CREATION FROM campagne");
